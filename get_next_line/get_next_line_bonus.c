...
static int	check(char **str, char	**temp, int fd)
{
	int		tempr;
...
char	*get_next_line(int fd)
{
	...
	while (r > 0)
	{
		...
		if (r == BUFFER_SIZE && tempr == -1)
			str = ft_strjoin_gnl(str, buff, r, &(temp[fd]));
		else if (tempr >= 0)
		{
			r = r - tempr - 1;
			temp[fd] = ft_subst_gnlr(buff, tempr + 1, r, &(temp[fd]));
			return (ft_strjoin_gnl(str, buff, tempr + 1, NULL));
		}
		else
			return (ft_strjoin_gnl(str, buff, r, &(temp[fd])));
		r = read(fd, &buff, BUFFER_SIZE);
	}
	return (str);
}
